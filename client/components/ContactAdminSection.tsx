  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex justify-center">
        <form onSubmit={handleSubmit} className="w-full max-w-3xl bg-white shadow-sm border border-input-border rounded-lg p-8">
          <div className="flex items-start gap-4 mb-6">
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-indigo-50 text-indigo-600">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 12v4m0 0v4m0-4h4m-4 0H8m8-8V4m0 0V2m0 2h4M4 6h16M4 10h16M4 14h16" />
              </svg>
            </div>
            <div>
              <h2 className="text-lg font-medium text-text-primary">Send a request to administrator</h2>
              <p className="text-text-muted text-sm">We will respond as soon as possible. Provide as much detail as you can.</p>
            </div>
          </div>

          {successMessage && (
            <div className="mb-6 px-4 py-3 bg-green-50 border border-green-200 text-green-800 rounded">{successMessage}</div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <span className="inline-block px-3 py-2 rounded-md bg-gray-50 border border-dashed border-input-border text-sm text-text-muted">Choose file</span>
                <input type="file" onChange={handleFileChange} className="hidden" />
                <span className="text-text-muted text-sm">{file ? file.name : "No file chosen"}</span>
              </label>
            </div>

            <div className="flex items-center justify-between pt-4">
              <p className="text-text-muted text-sm">For any questions regarding your verification or account, please contact your administrator through this form.</p>
